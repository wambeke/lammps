...
extern "C" {
  /*milady_force(i, nmax, eflag_either, eflag_global, eflag_atom, vflag_atom, &
            eng_vdwl, eatom, ntype, type, fmap, map_mass, &
...
/*  milady_setup_(&MiladyParams->nochannels,   &MiladyParams->nelements,  &MiladyParams->ninteractions, &MiladyParams->nparameters,
                &MiladyParams->parameters[0], &MiladyParams->rcutvec[0], &MiladyParams->ndetails,
                &MiladyParams->details[0], &MiladyParams->weights[0],  &MiladyParams->weights_3ch[0], &MiladyParams->ref_energy_pa[0], &MiladyParams->descriptor_type); */
  
  void milady_setup_(int *, int *, int *, int *,
                    double *, double *, int *,
...
class PairMILADY : public Pair {
 ...
 private:
  ...
  struct MiladyStruct {
    ...
    // in my version ninteractions will be allways 1 ///
    int ninteractions; // = nelements * (nelements+1)/2
    // COS for 1, 2 or 3 channels) //
    int nochannels; // 1 2 or 3
    ...
    double *weights; // vector of nelement weights 1 or 2  channels
    double *weights_3ch; // vector of nelement weights for 3rd channels
    double *ref_energy_pa; // reference energy per atom user
